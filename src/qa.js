...
export function run(from = 1, to = 10, opts = {}) {
  ...
  timer = setInterval(() => {
    ...
    if (target) {
      ...
      // clearance scales with range: point-blank shots need little margin, long shots more
      const clearR = Math.max(0.55, Math.min(1.3, dl * 0.05));
      for (const c of w.civilians) {
        if (c.dead) continue;
        const t = raySphere(eye.x, eye.y, eye.z, ux, uy, uz, c.pos.x, c.pos.y + 0.9, c.pos.z, clearR);
